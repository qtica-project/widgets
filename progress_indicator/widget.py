...
from ..tools.wrappers.progress_indicator import ProgressIndicatorWrapper
...
class _ProgressIndicator(QWidget):
    def __init__(self,
                 ...
                 step_size: int = 35,
                 font: QFont = None,
                 steps_content: list[ProgressIndicatorWrapper] = None,
    ...
    def _get_step_value(self, step: int):
        _step = self._steps_content[step - 1]
        if step == self.current_step:
            return _step.inactive
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        for step in range(1, self._max_step + 1):
            ...
            if (not self._is_steps 
                ...
                font = self._font
                font.setPixelSize(12 if step == self.current_step else 11)
...
class ProgressIndicator(AbstractWidget, _ProgressIndicator):
    def __init__(self,
                 *,
                 maximum: int,
                 current: int = 1,
                 step_size: int = 35,
                 font: QFont = None,
                 steps: list[ProgressIndicatorWrapper] = None,
                 style_color: QColor = None,
                 line_color: QColor = None,
                 text_color: QColor = None,
                 pen_width: int = None,
                 **kwargs):
        _ProgressIndicator.__init__(self, maximum, current, step_size, font, steps)
